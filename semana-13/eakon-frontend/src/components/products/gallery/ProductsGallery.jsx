...
const ProductsGallery = ({ products }) => {
  ...
  return products.length > 0 ? (
        ...
        return (
          <li key={element.id} className="card card--products">
            ...
            <div className="card__body card__body--products">
              <h3 className="card__subtitle card__subtitle--sm">
                {marca.data[0]?.attributes.nombre ?? "EAKON"}
              </h3>
              <Link
                to={`/productos/${slug}`}
                className="card__title card__title--sm"
              >
                {nombre}
              </Link>
              <h4 className="card__highlighted mt-auto">
                {peruvianCurrencyFormat(precio)}
              </h4>
              <a
                href={`https://api.whatsapp.com/send?phone=51${
                  contactData.attributes?.componentes[3].numero
                }&text=Deseo cotizar el producto ${nombre}, marca: ${
                  marca.data[0]?.attributes.nombre ?? "EAKON"
                }`}
                target="_blank"
    ...
    <h3 className="section__subtitle t-align-center f-elements f-elements--center">
